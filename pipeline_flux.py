...
class FluxPipeline(
    ...
    def __call__(
        ...
        #     latents,
        # )
        #X = torch.randn((1, 3, 1024, 1024), device=device).to(dtype=prompt_embeds.dtype)  
        ...
        if True:
                ...
                for indices in self.cfg.MODEL.T_LIST:
                        ...
                        timestep = self.scheduler.timesteps[indices].to(device=model_input.device)
                        timesteps = timestep.expand(model_input.shape[0])
                        # Add noise according to flow matching.
                        sigmas = self.scheduler.sigmas[indices].to(device=model_input.device)
                        ...
                        noisy_model_input_list.append(packed_noisy_model_input)
                        timesteps_list.append(timesteps)
                latents = noisy_model_input_list[0]
                timestep = timesteps_list[0]

                with self.transformer.cache_context("cond"):
                    features_t = self.transformer(      #only return one t feature for code simplicity
                        hidden_states=latents, #latents,
